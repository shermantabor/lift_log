"""
Menu option definitions for Lift Log.

Defines available menu actions and maps user selections
to their corresponding application behaviors.
"""
...
def start_new_session(user_id: int, notes=None) -> int:
    '''
    I: user id and notes
    P:  (1) close active session if one exists
        (2) start new session
    O: session id
    '''
...
def add_set_ui(user_id):
    '''
    I: user id
    P:  (1) ensure active session exists
        (2) ask user for sets to add to session
        (3) call parse_entry_line(), parse_set_token(), and db_insert_sets()
            to add sets to session
    O: none
    '''
...
def view_active_session(user_id):
    '''
    I: user id
    P:  (1) check if active session exists
        (2) pull sets from db for active session
        (3) display sets to user
    O: none
    '''
...
def view_stats(user_id):
    '''
    I: user id
    P:  (1) ask user for exercise to provide stats
        (2) check db for exercise
        (3) display exercise stats to user
    O: none
    '''
...
def view_sessions(user_id):
    '''
    I: user id
    P:  (1) pull all sessions from db
        (2) display sessions to user
    O: none
    '''
...
def end_active_session(user_id):
    '''
    I: user id
    P:  (1) pull active session from db
        (2) close active session if exists
    O: none
    '''
...
def closeout(user_id):
    '''end program, option to close open session'''
